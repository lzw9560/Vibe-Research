...
| 📈&nbsp;**推&#8288;荐&#8288;关&#8288;注** | 基于涨停基因得分的教育研究式关注清单（非交易建议）· 高质量 / 中等质量 / 低质量关注分级 |
| 🎯&nbsp;**战&#8288;法&#8288;信&#8288;号** | 8 大战法匹配（首板挖掘 / 连板接力 / 炸板回封 / 低吸龙头 / 反包战法 / N 字反击 / 平台突破 / 尾盘偷袭）· 入场价 / 止损 / 止盈参考 |
| ⚠️&nbsp;**风&#8288;险&#8288;仪&#8288;表&#8288;盘** | 个股风险量化（资金流 + 龙虎榜 + 波动率 + 回撤 + 流动性）· 风险分布 + 风险因素 TOP 10 · 动态阈值（基于市场情绪温度） |
| 🌡️&nbsp;**板&#8288;块&#8288;分&#8288;化&#8288;度** | 板块内部分化度 + 轮动速度监控 · 识别板块内部分化严重 / 轮动过快等市场特征 |
| 🚨&nbsp;**极&#8288;端&#8288;行&#8288;情** | 涨停潮 / 跌停潮动态阈值检测 · 基于历史数据自适应阈值，避免固定值误报 |
| 🌡️&nbsp;**情&#8288;绪&#8288;气&#8288;象&#8288;站** | 多因子市场天气评分（晴天/阴天/暴风雨）· 策略推荐 · 熔断规则 · 竞价指标 · 封单风险 · 仓位赦免管理 |
| ⏱️&nbsp;**打&#8288;板&#8288;工&#8288;作&#8288;流** | 盘前候选池 → 盘中实时信号/炸板预警 → 盘后结算复盘 · 状态机驱动 · 胜率追踪与战法调整建议 |
| 🔔&nbsp;**通&#8288;知&#8288;系&#8288;统** | 多通道推送（飞书 · 钉钉 · 邮件 · Telegram · Slack · Discord · 微信等 15+ 通道）· 统一路由与降噪 · 定时任务触发 |
| ⏰&nbsp;**定&#8288;时&#8288;任&#8288;务** | Cron 表达式调度 · 任务 CRUD + 手动触发 · 运行历史追溯 · 内置 6 种任务类型（数据刷新 / 复盘通知 / 基因预计算等） |
...
│   ├── trading_workflow.py  打板工作流编排器（盘前/盘中/盘后）
│   ├── workflow_state_machine.py 工作流状态机
│   ├── risk_models.py   风险模型（重构后）
│   ├── risk/            风险管理子模块（炸板预警 · 仓位管理）
│   ├── notification/    多通道通知系统（15+ 通道）
│   ├── strategies/      战法库
│   ├── settlement/      盘后结算引擎
│   ├── data_provider/   统一数据提供者抽象
│   ├── services/        服务层
│   ├── routers/         API路由（26+ 模块，含 workflow/sentiment_weather/scheduled_tasks）
