...
> - Q5=A 选股池 Tab 复用既有组件（FunnelLayers + SelectionPipeline），不新建组件（注：CandidateFunnelEmbed 是 PreMarketBriefing 局部函数不可复用，H1 改用 FunnelLayers + SelectionPipeline）
...
| R7 前端选股池 Tab | `frontend/src/lib/candidates.ts:122 candidatesApi.runFunnel/getFunnelLayers` API 已有；`components/candidate/FunnelLayers.tsx`（独立组件）+ `components/ui/FunnelLayerCard.tsx` 已实现；`SelectionPipeline`（Candidates.tsx 已有独立使用先例）。**注**：CandidateFunnelEmbed 是 PreMarketBriefing 局部函数不可复用，H1 改用 FunnelLayers + SelectionPipeline | `candidates.ts:122-141`；`FunnelLayers.tsx` |
...
| AC5a FunnelResult.market_context（市场宽度） | **移 backlog，不在本 spec 实现**（C 段 22 因子已移 backlog）。原复用：`market._emotion(date)` 返回 zt_count/zb_count/dt_count/max_boards/lianban_count/seal_rate/break_rate/promotion_rate/ladder/lianban_stocks；带日期参数可取昨日 | `market.py:116-266` |
| AC5b DiagnosisCard.seat_detail（龙虎榜席位） | **移 backlog，不在本 spec 实现**。原复用：`seat_engine/service.py:231 compute_consensus_signal(trade_date, stock_code)` + `hot_money_seats.py:321 compute_seat_risk_factor(code, trade_date)` | `seat_engine/service.py:231-364`；`hot_money_seats.py:62-70,321-410` |
| AC5c IndicatorSet 派生因子（封单变化率/N日涨幅/换手分位/板块连续流入） | **移 backlog，不在本 spec 实现**。原复用：`compute_trajectory(snapshots)`；K线 bars；`fund_flow.main_net_5d` | `intraday_features.py:61-92`；`activity.py:63` |
| AC5d IndicatorSet 加公告催化类型+概念联动 | **移 backlog，不在本 spec 实现**。原复用：`catalyst.py:19 classify_announcement(ann)` + `catalyst.py:53-56 concepts` | `catalyst.py:19-29,42-66` |
...
3. **match_strategies 当前内部已调 S070 R7**（`limitup_strategy.py:817-825`：`get_snapshots_by_code(code, 今日)` + `compute_derived_features`）。S084 改为从 `card.derived` 读（取 T-1 昨日值）。**card=None 时保留内部 fallback 路径不删**（Q3=C pre_market_workflow 不传 card 走 fallback 行为不变）；card 非空时 override 从 card 读（B2 决议：保留 fallback，不删）。

**新增**：DiagnosisCard 3 子对象 + IndicatorSet 6 扩展字段 + 2 个新 source 文件（zt_pool/derived）+ match_strategies 改从 card 读 + 前端两级 Tab。**不新增**：数据层（复用 astock/market/seat_engine）、漏斗筛选逻辑（不改 R1→R2→R3）、pre_market_workflow（Q3=C 保留）、market_context/seat_detail/派生/催化 source（AC5a-d 移 backlog，不在本 spec 实现）。
...
backend/
├── candidate_funnel/
│   ├── models.py                      # 【改动】DiagnosisCard 加 3 子对象；IndicatorSet 加 6 字段（FunnelResult.market_context 移 backlog 不加）
│   ├── diagnosis.py                   # 【改动】build_diagnosis_card 塞入 gene_score/pool_item/derived；build_indicator_set 透传 6 新字段
│   ├── sources/
│   │   ├── gene.py                    # 【改动】fetch_genes 扩展存完整 GeneScore 对象（gene_obj 键）
│   │   ├── activity.py                # 【改动】按历史日路径重写：kline prev_bar 复算 open/change_amt/last_close；tencent_quote 当日取 pe_ttm/mcap_yi/pb（标当前值非T-1）+ prev_amount_yi 从 K线前日 bar
...
│   │   # 注：market_context_source / seat_detail_source / 派生 / 催化 扩展（AC5a-d）移 backlog，不在本 spec 实现
│   ├── funnel.py                      # 【改动】_run_funnel_impl 采集 2 新 source（zt_pool/derived）+ 塞入 DiagnosisCard 3 子对象
│   └── tests/
│       ├── test_diagnosis.py          # 【改动】3 子对象填充 + 缺失降级
│       └── test_sources_contract.py   # 【改动】新 source 契约
├── limitup_strategy.py                # 【改动】match_strategies 改从 DiagnosisCard 读（新参数 card，默认 None 保留 fallback 向后兼容）
├── strategies/strategy_matcher.py     # 【改动】match/match_batch 加 card 参数
├── routers/
│   └── workflow.py                    # 【改动】_build_funnel_layers 透传 DiagnosisCard 3 子对象（funnel_mod.run_funnel 已含，透传即可）
...
frontend/src/
├── pages/
│   ├── Workflow.tsx                   # 【改动】加两级 Tab（选股池/战法），选股池 Tab 调 runFunnel + FunnelLayers + SelectionPipeline
│   └── workflow/PreMarketBriefing.tsx # 【不改】?strategy= 已实现（Q4=A）
├── components/candidate/
│   ├── FunnelLayers.tsx               # 【不改】既有漏斗各层卡片（Q5=A 复用，独立组件可复用）
│   └── DiagnosisCard.tsx              # 【改动】展示 3 子对象
├── components/ui/FunnelLayerCard.tsx # 【不改】单层展示
└── lib/
    └── candidates.ts                  # 【改动】DiagnosisCard 类型加 3 子对象；IndicatorSet 加 6 字段
```
...
class DiagnosisCard(BaseModel):
    ...
    # 注：seat_detail 子对象（AC5b）移 backlog，不在本 spec 实现
```
...
class IndicatorSet(BaseModel):
    ...
    # 取数路径分两路（B4 决议）：
    #   - kline prev_bar 复算（历史日可取）：open / change_amt / last_close
    #   - tencent_quote 当日取（标"当前值非T-1"）：pe_ttm / mcap_yi / pb
    last_close: Optional[float] = None       # 昨收；历史日=kline prev_bar.close，当日=vals[4]
    open: Optional[float] = None            # 开盘；历史日=kline bar.open，当日=vals[5]
    change_amt: Optional[float] = None       # 涨跌额；历史日=close-prev_close 复算，当日=vals[31]
    pe_ttm: Optional[float] = None           # 市盈率（vals[39]）；tencent_quote 当日取，标"当前值非T-1"
    mcap_yi: Optional[float] = None          # 总市值（亿）（vals[44]）；tencent_quote 当日取，标"当前值非T-1"
    pb: Optional[float] = None              # 市净率（vals[46]）；tencent_quote 当日取，标"当前值非T-1"
    ...
    # 注：AC5c 派生因子（seal_delta/change_5d/10d/20d/turnover_percentile_250d/sector_inflow_days）
    #     与 AC5d 催化因子（announcement_type/concept_count）移 backlog，不在本 spec 实现
```

### 2.3 FunnelResult 扩展

> **注**：`FunnelResult.market_context`（AC5a）移 backlog，不在本 spec 实现。本 spec 不扩展 FunnelResult 结构，3 子对象挂在 DiagnosisCard 上（final_candidates 已含）。

---

## 3. 实现步骤（按 R1-R9 顺序）

### 阶段 A：模型与 source 扩展（R1-R4）

> **注**：AC5a-d（market_context/seat_detail/派生因子/催化因子）移 backlog，不在本 spec 实现。阶段 A 只含 R1（gene_score）+ R2（pool_item）+ R3（derived）+ R4（IndicatorSet 6+3+1 字段）。
...
#### R4.1：IndicatorSet 加 tencent_quote 扩展 6 字段（按历史日路径重写，B4 决议）

**文件**：`candidate_funnel/sources/activity.py` + `diagnosis.py`

**按字段分路径**（B4 决议：不假设 activity.py 已调 tencent_quote 同一次返回扩展读 vals，按字段分历史日 kline 复算 vs tencent_quote 当日取）：

**路径 1：kline prev_bar 复算（历史日可取）—— open / change_amt / last_close**

`_fetch_activity_from_kline`（line 29-130，盘前取 T-1 走此路径）已有 `bar`（T-1 日）+ `prev`（T-2 日，line 72），扩展读：
```python
# bar = T-1 日 K线 bar，prev = T-2 日 bar（已定位，line 72）
entry["last_close"] = prev.get("close") if prev else None   # T-2 收盘 = T-1 昨收
if prev and prev_close:
    entry["change_amt"] = round(close - prev_close, 2)       # T-1 涨跌额
entry["open"] = bar.get("open")                              # T-1 开盘
```

**路径 2：tencent_quote 当日取（标"当前值非T-1"）—— pe_ttm / mcap_yi / pb**

tencent_quote 返回的是 T 日当前值，历史日取不到精确 T-1 值。从 `today_quote`（line 40 已调 `astock.tencent_quote(codes)`）取当前值近似，标 missing：
```python
# today_quote = astock.tencent_quote(codes)（line 40 已调，复用不重新调）
q = quote_from_tencent(c, today_quote.get(c, {}))
entry["pe_ttm"] = q.pe_ttm            # 当前值，标 missing["pe_ttm"]="当前值非T-1（tencent_quote 当日取）"
entry["mcap_yi"] = _numf_to_yi(q.market_cap)  # 当前值，标 missing
entry["pb"] = q.pb                    # 当前值，标 missing
```

**路径 3：limit_up / limit_down 复用既有字段（不新增）**

`IndicatorSet` 已有 `limit_up`/`limit_down`（models.py:38-39），`activity.py:164-165` 已从 `model.limit_up_price`/`model.limit_down_price` 赋值（§0 纠偏第 1 条）。本 spec 不重复加。

- **复用**：`astock.kline`（已调，bars 含 prev bar）；`astock.tencent_quote`（line 40 已调，复用不重新调）；`quote_from_tencent`（已映射）
- **关键**：按字段分路径——open/change_amt/last_close 走 kline 复算（历史日可取精确 T-1 值）；pe_ttm/mcap_yi/pb 走 tencent_quote 当日取（标"当前值非T-1"，诚实降级不臆造）；limit_up/limit_down 复用既有字段不新增
...
**测试要点**：
- 历史日路径 mock K线 bars，验证 last_close=prev.close / open=bar.open / change_amt=close-prev_close
- 验证 pe_ttm/mcap_yi/pb 在历史路径标 missing（"当前值非T-1"）
- 验证 limit_up/limit_down 复用既有字段（未新增）
...
#### R4.3 后续（AC5a-d 移 backlog，不在本 spec 实现）

> **B3 决议**：以下 4 项（market_context / seat_detail / 派生因子 / 催化因子）移 backlog，不在本 spec 实现。原实现步骤保留为 backlog 参考锚点，不在阶段 A 执行：
>
> - **AC5a market_context**：原计划新增 `market_context_source.py` 调 `market._emotion(yesterday_date)` 取市场宽度。**移 backlog**。
> - **AC5b seat_detail**：原计划新增 `seat_detail_source.py` 调 `seat_engine.compute_consensus_signal` + `hot_money_seats.compute_seat_risk_factor` 取龙虎榜席位明细。**移 backlog**。
> - **AC5c 派生因子**（seal_delta/change_5d/10d/20d/turnover_percentile_250d/sector_inflow_days）：原计划从已有数据派生。**移 backlog**。
> - **AC5d 催化因子**（announcement_type/concept_count）：原计划从 `catalyst.py` 已有数据扩展输出。**移 backlog**。
...
**向后兼容逻辑**（B2 决议：card=None 保留 fallback，不删）：
```python
# S084 R5：card 非空时从 card 子对象 override 读（不删 fallback 路径）
...
if card is not None:
    ...
    card_derived = getattr(card, "derived", None)  # 非空时 override derived
    card_gene = getattr(card, "gene_score", None)
    # gene_score 子对象可重建 GeneScore（若调用方只传 card 不传 gene）
# 注：card=None 时 card_derived 为 None，下游 PRD 战法走既有 fallback 路径（不删）
```

**既有 9 战法**（line 711-802）：从 `gene` 读（不变，gene 是 GeneScore 对象）→ S084 时 `gene` 可从 `card.gene_score` 重建（调用方负责）

**PRD 弱转强接力**（line 808-884）改动（B2 决议：保留 fallback，card 非空时 override）：
- **保留**既有内部 S070 取数 fallback（line 817-825：`get_snapshots_by_code(code, 今日)` + `compute_derived_features`）——card=None 时走此路径（Q3=C pre_market_workflow 不传 card 行为不变）
- **card 非空时 override** 从 `card.derived` 读：`derived = card_derived if card_derived is not None else <fallback 既有 S070 取数>`
- `card.derived` 非空：`broken_duration_min = derived.get("broken_duration_min")`，`max_drop_pct = derived.get("max_drop_pct")`，`last_lock_time = derived.get("last_lock_time")`
- `card.derived=None`（盘前未采集）且 card 非空 → 标 `s070_status="missing_s070_r7"` 跳过（既有逻辑，line 832-839）
...
def match_batch(
    ...
    for gene in genes:
        ...
        results[gene.code] = self.match(gene, weather_state,
            ...
            card=card)
```
- **向后兼容**（B2 决议）：`card` 默认 None，既有调用不传 card → 走既有 fallback 路径（Q3=C pre_market_workflow 不传 card 行为不变）；card 非空时 override 从 card 取全部子对象传给 match_strategies
...
- **选股池 Tab**（新增视图）：
  ...
  - 用既有 `FunnelLayers` 组件（独立组件，`components/candidate/FunnelLayers.tsx`）展示 R1→R2→R3 三层（Q5=A 复用，不新建组件）
  - 用既有 `SelectionPipeline`（`Candidates.tsx` 已有独立使用先例）展示 final_candidates（DiagnosisCard 列表含 3 子对象）。**注**：CandidateFunnelEmbed 是 PreMarketBriefing 局部函数不可复用，改用 FunnelLayers + SelectionPipeline
...
export interface DiagnosisCard {
  ...
  // 注：seat_detail（AC5b）移 backlog，不在本 spec 实现
}
export interface IndicatorSet {
  // ... 既有字段
  // S084 R4.1（按历史日路径分：kline 复算 vs tencent_quote 当日）
...
// 注：FunnelResult.market_context（AC5a）移 backlog，不在本 spec 实现
```
...
- `_build_funnel_layers` 调 `funnel_mod.run_funnel` 已返回含 3 子对象的 FunnelResult，透传即可
...
| AC5：IndicatorSet 含 12 扩展字段 | R4.1（6 字段）+ R4.2（3 字段）+ R4.3（1 字段） | **纠偏**：limit_up/limit_down 已有，实际加 6+3+1=10 字段（spec 12 含纠偏前口径）；按历史日路径分：open/change_amt/last_close 走 kline 复算，pe_ttm/mcap_yi/pb 走 tencent_quote 当日（B4） |
| AC5a：FunnelResult.market_context | **移 backlog** | 不在本 spec 实现（C 段 22 因子已移 backlog） |
| AC5b：DiagnosisCard.seat_detail | **移 backlog** | 不在本 spec 实现 |
| AC5c：IndicatorSet 4 派生因子 | **移 backlog** | 不在本 spec 实现 |
| AC5d：IndicatorSet 公告催化 + 概念联动 | **移 backlog** | 不在本 spec 实现 |
| AC6：match_strategies 从 DiagnosisCard 读，card=None 保留 fallback | R5（limitup_strategy.py 改） | card 非空时 override 从 card 读；**card=None 保留既有 fallback 路径不删**（B2：S070 自取数/kline_rebuild 调用保留，pre_market_workflow 不传 card 走 fallback） |
| AC7：既有 9 战法回归通过（传/不传 card 命中一致） | R5+R6（向后兼容） | card=None 时既有 9 战法命中不变（走 fallback）；card 非空时 override 从 card 读命中一致 |
| AC8：前端 Workflow.tsx 两级 Tab，选股池 Tab 调 runFunnel | R7（Workflow.tsx 改） | 选股池 Tab 用 FunnelLayers + SelectionPipeline 展示漏斗 R1→R2→R3 + final_candidates；调 `/workflow/candidates/funnel`（非 `/api/workflow/pre-market`）；CandidateFunnelEmbed 不可复用（H1） |
...
- **em_get 防封底线**：涨停池原始 dict（zt_pool_source）走 `astock.em_zt_topic_pool` → `em_get`（0.3s 限流 + circuit_breaker）；不直 requests
- **不臆造**：3 子对象各自管理缺失——`gene_score=None`（get_screener_result 返空）/`pool_item=None`（个股不在昨日涨停池）/`derived=None`（snapshots 未采集），各标原因
...
- **向后兼容**（B2 决议）：match_strategies/StrategyMatcher.match 新参数 `card` 默认 None；**card=None 时保留既有 fallback 路径不删**（S070 自取数/kline_rebuild 调用保留，Q3=C pre_market_workflow 不传 card 走 fallback 行为不变）；card 非空时 override 从 card 读；DiagnosisCard 3 子对象默认 None，既有快照无字段降级
...
| 风险 | 影响 | 缓解 |
|---|---|---|
| DiagnosisCard 加 3 子对象破坏序列化 | 前端类型/快照不兼容 | 3 字段默认 None，既有快照无字段降级（Pydantic Optional） |
| match_strategies 改从 card 读破坏既有 9 战法 | 既有战法不命中 | B2 决议：新参数 `card` 默认 None，**保留既有 fallback 路径不删**（S070 自取数/kline_rebuild 调用保留）；card=None 走 fallback 行为不变，card 非空 override 从 card 读（AC7 回归） |
...
**回滚**：
1. DiagnosisCard 3 子对象默认 None，不影响既有序列化（删字段即回退）
2. match_strategies 新参数 `card` 默认 None，删即回退（fallback 路径保留，删 card 参数后既有路径仍可用）
3. 前端两级 Tab 独立于既有 PreMarketBriefing，回滚只删 Tab 不影响盘前简报
4. 2 个新 source 文件（zt_pool/derived）独立，删文件即回退（不影响既有 funnel 筛选逻辑）
