...
- [x] A7：0819 真数据回测（2026-08-21）——用 0818 外围+内部真数据预测 0819，概率分 **49.7**（外围 72.3 大跌+内部 51.7 见顶真，新闻 16.7 失真 cache 缺历史，日历 30）。诚实未达 spec 原"高"(≥50)：①新闻 0818 缺（fetch_radar 当前 cache 不含历史 0818，fallback 0821 失真拉低）②模型对 0818"连板4板非极高见顶"不判高概率。外围+内部真数据对（暴风雨前兆命中），新闻失真 + 模型局限致 49.7。真数据源：push2his 并发 6/9 指数 + datacenter SOX + sti_timeline DB + gene_scores DB。脚本 tools/s088_a7_backtest.py。
