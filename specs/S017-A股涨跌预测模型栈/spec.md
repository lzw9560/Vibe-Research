...
- [ ] 预测输出卡片挂「历史统计特征，研究参考，不构成投资建议」声明
- [ ] 不承诺收益 / 不做确定性保证 / 不代用户决策（给概率与区间，不给"必买"指令）
- [ ] 判断可复现（公开数据+固定规则，`financial_rigor.py` 可核对），不臆造、不心算
- [ ] 涨停四池原始个股名仅聚合成无个股名情绪指标喂模型；个股预测头输出是预测结果非情绪聚合，不违反四池聚合规则
- [ ] `chat.SYSTEM_PROMPT` 新边界未被破坏（可给研究性研判，不承诺收益/不代客决策）
- [ ] 模型产物（权重/快照）存 VR_DATA_DIR，不进 git/不上传
- [ ] 新增东财端点走 `em_get()` 限流
- [ ] S4 盘中仅出教育性研判框架，不做自动信号推送/不下单/不代决策（半自动化：系统打分、用户决策）
