...
| ID | 任务 | 依赖 | 验收 |
|---|---|---|---|
| T1 | 扩展 `_should_run` 支持 `*/n`/范围/步进 | — | 单测覆盖边界 |
| T2 | 🩹补 `scheduled_tasks.py` 的 `timedelta`/`asyncio` import | — | limitup_precompute/cleanup 不 NameError |
| T3 | 🩹修 `TaskExecutor` 重复 `add_run`（开头 running，finally update 终态） | — | 一次执行一条 run 记录 |
| T4 | SQLite WAL+busy_timeout+连接复用 | — | 无 database is locked |
| T5 | 任务去重（执行前查 running 同任务） | T3 | 重复触发跳过 |
| T6 | FastAPI lifespan + `asyncio.create_task` 挂主循环 | — | 废线程内 asyncio.run |
...
| T14 | 单测：cron/TaskExecutor/状态机/lifespan | T1-T11 | 全过 |
