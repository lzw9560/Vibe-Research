...
| ID | 任务 | 依赖 | 验收 | 状态 |
|---|---|---|---|---|
| T1 | 扩展 `_should_run` 支持 `*/n`/范围/步进 | — | 单测覆盖边界 | ✅ d5b8e3e |
| T2 | 🩹补 `scheduled_tasks.py` 的 `timedelta`/`asyncio` import | — | limitup_precompute/cleanup 不 NameError | ✅ d5b8e3e |
| T3 | 🩹修 `TaskExecutor` 重复 `add_run`（开头 running，finally update 终态） | — | 一次执行一条 run 记录 | ✅ d5b8e3e |
| T4 | SQLite WAL+busy_timeout+连接复用 | — | 无 database is locked | ⬜ |
| T5 | 任务去重（执行前查 running 同任务） | T3 | 重复触发跳过 | ✅ d5b8e3e |
| T6 | ~~FastAPI lifespan + create_task 挂主循环~~ → **线程内长生命周期事件循环**（`_loop` 持 loop + `_ticker` 心跳） | — | 废线程内 asyncio.run；spawn 任务不被取消 | ✅ d5b8e3e |
...
| T14 | 单测：cron/TaskExecutor/状态机/lifespan | T1-T11 | 全过 | ✅ d5b8e3e（cron/TaskExecutor 47 例；状态机/lifespan 待 T10/T11） |
