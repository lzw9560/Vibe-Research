...
| S199 | S194 重测（方向感知编码） | no_contribution 真因=方向无关编码(GAP_REGIME_ENCODE 丢 direction+gap_direction 没喂 cases_vals)+常数权重;重测方向感知 gap 编码+衰竭重标(待S198)+regime 分拆+multifactor null | ⏳ spec 草案落 | 不预断有 edge(verdict),重测可能仍 no edge;R1 方向感知独立,R2 衰竭重标阻塞待 S198 verdict;实现待 S198+数据 |
| S200 | A股量化投研知识图谱体系 | 4破局哲学(微观结构VPIN/预期差/CentralRouter虚拟路由/动态断路器)+五域(政策/产业/资金/学术/策略)+跨频次调度+Vibe-Research落地 | ⏳ spec 草案落 | 方法论骨架+五域实体+risk_model+action;4方法论子spec S201-S204待起;quant-system-brainstorm workflow综合后定稿;关联4痛点memory+[[S198]]+[[S199]] |
| S201 | CRITICAL bug 修复(拆3子spec) | S201a cost可复现(rewire F3逐笔_cost_pct保留5元门非flatten)+S201b cost校准(降spread0.70→0.10-0.20%+修出场乐观+扫14脚本+重结journal)+S201c gap前视(candidate语义去_is_filled+修arity bug+重建case库) | ⏳ spec v2 verdict修订(15条,w7i1bugmm needs-revision) | 撞判断须可复现底线;S201a立即单文件低爆炸半径;S192不翻案(3否定cost-independent);S200不阻塞(3子实体标pending);两bug不捆绑+large级走SDD+feature分支 |
