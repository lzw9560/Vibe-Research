...
§44 grill reframe 结论：public EOD 因子（8 涨停史 1.6x、板块热度 1.2x）对"次日涨停/溢价"全 <2x（未 validated）。
涨停是**催化/资金驱动**，edge 若存在只在 **intraday ephemeral 数据**（封单动量、实时资金流、tick）里——
但当前 intraday 数据近期-only（封单 2 日 / 资金流 1 日）→ §44 验证日历阻塞（60日复验窗口下，数据不足无法出 validated 结论，但不阻断管道建设跑通）。
...
- §44 60日复验窗口：当前 intraday 数据近期-only（封单 2 日 / 资金流 1 日），§44 验证日历阻塞——管道仍接入跑通（trajectory 是未测的不同类，有破 2x 可能），标注"§44未验证/探索性"，等 30 日后数据积累验证，破2x→validated 接选股，<2x→标噪声保留接入。
...
- **验证（盘后，~30 日后）**：R4 §44 60日复验窗口验证 intraday 因子 → 次日涨停/溢价。复用 sector_heat_validation 口径
  （热/冷分位 + Wilson CI + lift）。验证前管道接入跑通标"未 validated/探索性"，验证后破2x→validated，<2x→标噪声保留接入。

**先验诚实**：§44 public EOD 全死的模式（<2x 标未 validated，不阻断接入）暗示 intraday 也可能 <2x。R1（trajectory）零成本（导出现有 snapshots）
