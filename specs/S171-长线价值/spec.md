...
**窗口+universe 双变更不可分离**：S168→S171 同时改了窗口（3 天→1 月）和 universe（涨停→全 A）。若 S171 发现 edge，无法归因于窗口变更还是 universe 变更。spec 承认此 confound，不声称"证否 S168 错窗口"。
...
**winrate vs mean（关键方法论）**：§44v2 verifier 的 selection verdict 用 `winrate_lift_avg` 驱动 status（verifier.py:283 实测确认），但 value premium 是 **mean-return 现象**（价值股赢的频率不一定更高，但平均收益更高 via 尾部）。`mean_lift_avg` 已计算（stats.py:231）但**从不用于 status 判定**。winrate lift 对 value 结构性 ≈1.0-1.2，永远到不了 2.0x robust_edge 门槛——会假阴性。spec 因此以 **event edge_type（mean excess return t-test）为 PRIMARY**，selection verdict 降为 auxiliary（标"winrate 指标对 value 结构性受限"）。

## 1. 需求清单

- [ ] R1 数据采集 scan_long_value_cache.py：分 Layer0-4 background 跑（各自独立可 resume + checkpoint）
  - Layer0（exploratory pre-check，可选）：akshare stock_value_em 全股 × 2018-2026 daily PE/PB/PS（~2.5hr，PIT-clean 已验 epsTTM 在 pubDate 跳变），标 survivorship_biased=true（退市股返 None）。**先跑 stock_value_em 出快速初判**——若初判 falsified 则 Layer1-4 可省 ~11hr；若 positive 则 baostock 确认
...
- [ ] R3 §44v2 wire（PRIMARY=event mean-based，AUXILIARY=selection winrate）：
  - **PRIMARY**：Q1-Q5 long-short spread，edge_type="event"，returns = [bottom_quintile_month_ret - top_quintile_month_ret per month]，dates = [month_ISO...]。canonical Fama-French HML 测试（beta 中性——同月双臂市场 beta 对消；mean-based——t-test 检验 mean spread > 0）。harness 预扣成本：returns[i] -= 2 × round_trip_cost × turnover_i（long+short 双腿）。n_comparisons=4（Family A）
  - **SECONDARY**：Q1 excess over universe mean，edge_type="event"，returns = [Q1_month_ret - universe_mean_month_ret]。long-only 可实现版，beta 中性。n_comparisons=4（Family A）
  - **AUXILIARY**：selection low_pe + high_pe，edge_type="selection"，survivors_by_month/universe_by_month，window_sanity={"path": {mean, winrate, base_rate}}。R5 前置 sanity 检 path 窗口——value 的 winrate 可能 ≈ universe → R5 触发 exploratory（**预期行为非 bug**——selection winrate 对 value 结构性受限）。n_comparisons=4（Family A）
...
- [ ] R5 survivorship / lookahead / cost / regime 处理：
  - **survivorship = PARTIALLY correct**（非"survivorship-correct"）：baostock 覆盖 **多数** 退市股活跃期 kline（实测 sz.000033=364 bars, sh.600432=364 bars）但 **非全部**（sz.000405 返 0 bars）。R1 Layer3 coverage 审计统计覆盖率。<100% 则 caveat residual survivorship bias（value trap 缺失方向 = **高估 value premium**）。verdict 须标 "survivorship-partially-correct, bias direction: favors value"
  - **退市 return = anti-conservative for value**（非"保守"）：退市整理期实际损失 -50%~-90%（老三板 NEEQQ），用 outDate 前最后 close 仅计 -5%~-10% → 系统性高估 value premium。spec 采用 **-100% worst-case**（退市月 return = -1.0，归零）+ sensitivity（-50% vs -100% 看 verdict 是否翻转）。退市 return 规则在 survivors_by_month 和 universe_by_month **一致**（同月同值，universe 不静默 drop 退市股——否则 base_rate winrate 被偏移）
...
**张力5 winrate vs mean**（关键）：selection verdict 用 winrate_lift_avg 驱动 status（verifier.py:283 实测确认），对 value 结构性 ≈1.0-1.2（赢的频率不更高，但均值更高 via 尾部）→ 永远到不了 2.0x robust → 假阴性。spec 以 **event edge_type（mean excess return t-test）为 PRIMARY**：Q1-Q5 long-short spread（canonical HML，beta 中性 + mean-based）+ Q1 excess over universe mean（long-only 可实现）。selection verdict 降为 auxiliary。**不改 verifier**（不改 selection_lift = winrate_lift_avg→mean_lift_avg）——YAGNI，用 edge_type=event 绕过即可。
...
**张力9 月度 autocorrelation**：月度 value portfolio 组合慢变（同股连续月在 bottom-quintile），跨月自相关违反 iid 假设——day_clustered t-test 的 se = day_std/sqrt(n_days) 假设月均值独立，自相关时 t 略偏高。selection permutation（月内 cross-sectional）不受影响。caveat 标注，PurgedKFold 补充 OOS。
...
- [ ] A2 long_value_run 出 verdict：Family A（K=4）= Q1-Q5 spread + Q1-excess-universe + selection-low_pe + selection-high_pe；Family B（K=1）= Q1-excess-HS300；Family C（K=1）= PE+ROE spread（若数据够）。全落 Recorder + lineage
- [ ] A3 PRIMARY verdict edge_type=="event"（Q1-Q5 spread），AUXILIARY verdict edge_type=="selection"；days_robust ≥ 60 或诚实标 underpowered
- [ ] A4 退市股 -100% worst-case + sensitivity（-50%）跑两遍，verdict 一致或注明翻转
