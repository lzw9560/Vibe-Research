...
- [ ] R1 数据采集 scan_long_value_cache.py：分 Layer0-4 background 跑（各自独立可 resume + checkpoint）
  - Layer0（exploratory pre-check，可选）：akshare `stock_value_em(symbol=code)` **单股接口**（非全股一次，须遍历 ~5000 股）× 2018-2026 daily PE/PB/PS（~2.5hr，**PIT-clean 已验数据源可对齐**——subagent A 9-12 实测 600519/000001 有 7.5 年日频 PE + baostock pubDate 两边都有；精确对齐脚本收紧跳变点阈值到 pubDate ±2 日是 Layer0 落地子步骤，gate 确认可行）。标 survivorship_biased=true（退市股返 None）。**决策规则**：Layer0 positive → baostock 全量确认；Layer0 falsified → **不直接省 baostock**，先 spot-check（抽 N 股含退市，baostock PIT 重建 PE 重算 Q1-Q5 spread，看跟 akshare 结论翻不翻——翻=偏差假阴性→跑全 baostock；不翻=falsified 可信→省 baostock）。双偏差（survivorship+lookahead）下 falsified 不可直接信
  - Layer1 kline 多年：baostock query_history_k_data_plus 全股 × **2016-01-01..2026-09-03**（~129 月，过 R6 60 门槛；2016+ 给 PIT earnings 4Q lookback），fields=date/open/high/low/close/volume/amount/turn/pctChg/isST。每 50 股 re-login（复用 first_board_premium_baseline.py RELOGIN_BATCH=50 + _bs_code），存 baostock_kline_cache_long.json（不覆盖现有 ~9 月 cache）。**前复权 PE 致命 bug（承重）**：adjustflag=2 前复权调历史价到当前股本，但 epsTTM 是 as-reported 原始股本 → PE=前复权价/原始 eps 系统性虚低 → 送转多的成长股误入 Q1 value → 稀释 value premium（10 年全 A 送转极普遍，影响致命）。**fix**：PE 须用 `adjustflag=3` 不复权 close 算（或 basis-adjusted close(D)×shares(D)/shares(pubDate)），return 仍用 adjustflag=2 前复权 total return（含 split/dividend 经济回报）——selection 用不复权 PIT-correct PE、return 用前复权 total return 是可辩护设计，须验证 Q1 quintile 跨调整法 stability。须同时存 adjustflag=2（return 用）和 adjustflag=3（PE 用）两套 cache
  ...
  - Layer3 PIT universe + 退市：baostock query_all_stock(月末交易日) 重建每月 PIT active 集（主源，**PIT 行为未验**——query_all_stock(day) 返历史时点 active 集还是当前快照未验；若返当前快照→历史月缺退市股=survivorship bias→verdict 无效。**Layer3 第一步 gate**：抽查≥3 历史月末 query_all_stock(D) ∩ stock_basic(outDate>D) 须非空，不含即将退市股→fallback stock_basic outDate+ipoDate 重建 ipoDate<=D AND (outDate>D OR outDate="")）。**query_all_stock 返所有证券含指数/债券/ETF 非仅 A 股股票**（baostock metadata 返"给定日期所有证券信息"）→ 须与 stock_basic type=1 集交集过滤。query_stock_basic **仅 type=1 过滤**（8945 全表含指数/债券非全股票——实测确认）取 outDate 元数据（补充）。退市股 kline 按 ipoDate..outDate 取活跃期。**outDate='' 二义（承重）**：可能是活着的股（从未退市）OR baostock 漏标的退市股（outDate 未填）→ fallback 可能把漏标退市股当活着注入所有历史月→universe 含已退市股虚假 return。sanity：outDate='' 且 kline 最后 bar 远早于 now（>6 月无新 bar）→ 标'疑似漏标退市'不入 universe。须验 stock_basic 退市股数 vs 已知 A 股退市股总数（~300+），漏标率高则 fallback 不可信，须用 akshare stock_info_sh/sz_delist 交叉验 outDate。**0 bars 退市股不可恢复 bias（承重）**：0 bars 退市股无 kline→无法确定哪些月 active→无法 inject -100%→不可恢复 survivorship bias（方向**确定=高估 value premium**：0-bars 股=长期停牌后退市=value trap 极端，缺失→Q1 收益虚高）。须统计 0 bars 退市股数及占比，>5% 标'unrecoverable survivorship bias'降级 verdict；严格覆盖率 gate 把 0 bars 股计入分母（算 0% 覆盖）非静默排除。**停牌股 stale close（承重）**：query_all_stock(D) 返 D 日 listed 含停牌股（baostock 无 tradeStatus 字段无法直接过滤）。停牌股 close(D)=最后成交价（可能数日/数周前）非 D 日出清价→PE=stale_price/epsTTM→quintile 错配（真实价已跌但停牌→stale 高价→低 PE→误入 Q1 value）。universe_by_month 须过滤 D 日停牌股（kline(D) volume==0 或无 D 日 bar）从 universe 和 Q1/Q5 候选双重剔除。A 股停牌频繁（*ST/退市前尤甚，与 Q1 value 高度重叠）→ 影响面大且方向偏 value false-positive。**退市 coverage 审计**：对全部 outDate!="" 的 type=1 退市股试取活跃期 kline，统计覆盖率（subagent A 9-12 抽样 60 只退市股实测宽松覆盖 100% + 严格覆盖 100%（活跃期≈93%>80%），sz.000405 实测 1869 bars 非 0（spec 原 0 bars 担心不成立）→ 退市覆盖可信）。存 historical_universe_monthly.json + stock_basic_type1.json
...
- [ ] R2 factor 定义（PIT bottom-quintile，3 因子）：
  - F1 Low PE（earnings yield）：月底 rebalance 日 D = **月末最后交易日**（baostock query_trade_dates，非 calendar 月末——周末/假日无 close），PE = **不复权 close(D)** / PIT_epsTTM（**pubDate < D 严格小于**——A 股年报/季报常盘后披露，`<=` 允许同日=用盘后信息配盘前价=lookahead，~25% 月受影响。复用 multifactor_combo_test.py:141 compute_pe 须改 `<=` 为 `<`）。**compute_pe sort tie-breaking（承重）**：compute_pe line 144 `valid.sort(key=lambda x: x[1]["pubDate"], reverse=True)` Python stable sort 同 pubDate 时取原 dict 顺序（Q4 排 Q1 前→取 Q4 epsTTM 非 Q1）。get_pit_profit_row helper 须加次级排序键 `(pubDate, quarter_key)` 降序，quarter_key parse 为 `(year, quarter_num)` 元组降序（非字符串排序，靠年份编码运气脆弱）。不复权 close 见 Layer1 前复权 PE 致命 bug fix——PE 用不复权/basis-adjusted 非 adjustflag=2 前复权。取 bottom-quintile（最低 PE=最高 earnings yield）+ top-quintile（对照）。**显式标注**：compute_pe 过滤 `epsTTM > 0`（multifactor_combo_test.py:141 实测确认），排除亏损股（实测 ~32% 被排除），排除方向 = 高估 value premium（移除 value trap）。**universe_by_month 从 ALL PIT active 股构建**（含亏损股），非仅正盈利股——否则 universe 也被正向偏移。无 PIT earnings（pubDate<D 无 epsTTM 或 epsTTM<=0）的股票从 PE ranking **排除**（标 "no PIT earnings, excluded from PE quintile"），统计排除率——若某月 >30% 排除则标该月 low-coverage
...
- [ ] R3 §44v2 wire（双 co-PRIMARY 互验 + AUXILIARY selection）：
  ...
  - **SECONDARY-benchmark**：Q1 excess over HS300，edge_type="event"，returns = [Q1_month_ret - HS300_month_ret]。caveat: "size-confounded（HS300 大盘，value 偏中小盘）——非 pure value edge"。n_comparisons=1（Family B，独立）。**BH m=1 bug caveat（latent）**：verifier.py:364 event_status 用 `p_bh`，但 stats.py:355-357 BH 分支 `m=len(p_values)=1`→零校正（`m<=1 → return [min(p,1.0)]`）。SECONDARY 当前 K=1 → p_bh=p_bonf=p（moot，无 bug）；**若扩 K=2（加 ZZ500 等）须 harness 层 enforce p_bonf<0.05**（即 p_one_sided<0.025，K=2 Bonferroni p×2）——stats.py BH per-call 对 K=2 仍返零校正（m=1），reproduce 不一致。co-PRIMARY ① K_event=1 同 moot。AUXILIARY selection 分支用 `p_bonf`（stats.py:348-352 正确 p×k）不受影响
  - Family A K=4（4 测试同族 Bonferroni/BH 校正）；Family B K=1；F2 PE+ROE K=1（Family C）。全 K ≤ cap 8
  - wire_verdict 新增 **5** 参数透传：window_sanity=None / walk_train=None / walk_test=None / step=None / **event_materiality_floor=None**。None 时 verify() 用日频默认值（保 S168/S169/S170 向后兼容）。S171 harness 传 monthly 值：walk_train=36, walk_test=12, step=12, **event_materiality_floor=0.001**（月频校准——默认 0.003 是日频 0.3%/天=3.6%/年门槛，对 A 股 value premium 2-4%/年恒 event_thin_positive 非 robust_edge；0.001=0.1%/月=1.2%/年匹配 A 股价值溢价量级）。**event_materiality_floor 须存入 Recorder params dict 才能 reproduce**（verifier.py:370-374 `if days_robust>=60 and t_res.day_mean > effective_floor: event_robust else event_thin_positive`——effective_floor=max(event_materiality_floor, round_trip_cost*0.5)，是唯一直接影响 status 的参数，不存→reproduce_verdict 用 verify 默认 0.003 非 harness 0.001→status 翻转→A5 炸）。**不加 periods_per_year**（verify():169 声明但 body 从未使用——dead param，实测 grep 确认）
...
- [ ] R5 survivorship / lookahead / cost / regime 处理：
  - **survivorship = PARTIALLY correct**（非"survivorship-correct"）：baostock 覆盖 **多数** 退市股活跃期 kline（实测 sz.000033=364 bars, sh.600432=364 bars）但 **非全部**（subagent A 实测抽样 60 只退市股 100% 有 kline（sz.000405=1869 bars 非 0），退市 334 个）。R1 Layer3 coverage 审计报**双数**：宽松覆盖率（返>0 bars 占退市股 %）+ 严格覆盖率（ipoDate..outDate 完整活跃期覆盖≥80% 占 %）。verdict caveat 用**严格覆盖率** gate：严格<50% 标"退市处理不可靠，verdict 降级 exploratory"。<100% 严格覆盖 → residual survivorship bias（value trap 缺失方向 = **高估 value premium**）。verdict 须标 "survivorship-partially-correct, bias direction: favors value"
  ...
  - **lookahead**：PIT earnings gate 在 harness 层 enforce（pubDate **< D** 严格小于的最新 quarter epsTTM，复用 compute_pe 须改 `<=` 为 `<`）。**restatement risk** caveat：baostock 可能存储 **restated**（修正后）epsTTM 但保留原始 pubDate——pubDate<D 保证报告已公开，不保证值是 as-originally-reported。spec 加抽样验证步骤（5-10 stock-quarters 对比 as-reported vs baostock 值），偏差大则降级 verdict 为 exploratory
...
| `backend/tools/_s44_wire.py` | 改：wire_verdict 加 **5** 参数透传（window_sanity/walk_train/walk_test/step/**event_materiality_floor**），None=日频默认保向后兼容。**event_materiality_floor + survivors_by_day/universe_by_day 须存入 Recorder params dict**（event_materiality_floor 是唯一直接影响 status 的参数，verifier.py:370-374，不存→reproduce_verdict 用 verify 默认 0.003 非 harness 0.001→status 翻转→A5 炸）。**不加 periods_per_year**（verify():169 声明但 body 从未使用——dead param，传了无效果） |
...
**张力2 PIT earnings**：baostock profit_data 有 pubDate（PIT 锚点）+ epsTTM。harness 层 enforce pubDate **< D** 严格小于最新 quarter（盘后披露同日=lookahead）。restatement risk caveat（baostock 可能存 restated 值）。akshare stock_value_em PIT-clean **待验**但 survivorship-biased → Layer0 exploratory pre-check（快速初判）+ baostock 正式 verdict。
...
- [x] 判断可复现：§44v2 verifier = 复现机制（reproduce_verdict 重算）；baostock 真实 kline + profit_data，PIT earnings gate（pubDate **< D** 严格小于），退市 -100% + sensitivity，不假收益。financial_rigor.py 不适用（非市值/估值验算）
