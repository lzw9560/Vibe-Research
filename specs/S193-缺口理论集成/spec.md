...
- [ ] R5：分类准确率验（Q13-B，不是 §44 买卖 lift，**spec grill MEDIUM#5 修订——分负信息 gate vs 无 edge**）
  - 验"分类准不准"：突破缺口后 N 日（3/5/10 多窗口）是否真趋势启动（收益 > 阈值）
  - 混淆矩阵：判突破实际是持续的 % + 衰竭实际是普通噪声的 %
  - **两类区分（spec grill MEDIUM#5）**：
    - **负信息排除 gate**：分类准确率 **< 基率（如 < 33% 三分类基率）= 标签本身错（喂负信息）**→ **不进融合池**（不是"无 edge"，是"负 edge/噪声注入"，Q13"单信号无 edge ≠ 融合无价值"只覆盖"标签对但无收益预测力"，不覆盖"标签错"）
    - **无收益 edge 留 F1**：准确率 > 基率但无收益 lift → 标"分类对但无收益 edge"，**不阻止集成**（Q13 反思成立），留 S194 F1 消融验增量
  - 准确率 < 基率 → 排除 gate（不进池）；> 基率但无收益 lift → 不阻止集成（留 F1）
