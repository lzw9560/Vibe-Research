...
- §1 合规：私有 key 只存 `~/.vibe-research/`（`VR_DATA_DIR`），绝不进 git。
...
- [ ] R2 `get_fred_api_key() -> str | None`：从 `~/.vibe-research/fred_api_key`（VR_DATA_DIR）读 key，缺失返 None。key 绝不进 git、不打日志。
...
- [ ] R5 注册的特征**不加入** HEAD_FEATURE_SUBSETS["short_sector"]，直到 key 到位 + live 冒烟通过（守"未补前不入短线头"）。
...
- key 读取：`get_fred_api_key` 读 `Path(os.environ.get("VR_DATA_DIR", Path.home()/".vibe-research")) / "fred_api_key"`。
...
- [ ] A5 macro 特征未加入任何 HEAD_FEATURE_SUBSETS（短线头未泄入）
- [ ] A6 `pytest -m "not live"` 全过

## 7. 合规自查（CLAUDE.md §1）

- [ ] key 只存 ~/.vibe-research/，绝不进 git/日志/异常
