...
- **gap 是 event edge 非 selection edge**（v2 修，治 grill honesty lens）：gap run 须出 **两个独立 verdict**：
  - **population event-edge verdict**（全涨停 gap return series vs base rate，K=1，DSR n_trials=1，edge_type=event）——回答 §3 标题"验证 gap 是否 edge"，用 **day-clustered one-sample t-test（mean gap>0 after costs?）+ binomial（WR>50%）**，**非 lift/permutation/Bonferroni（测 selection）**。
