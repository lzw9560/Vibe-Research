...
- **codify ad-hoc skill 成可复用 workflow**（2026-09-15 加入项目实践，延伸 §7 提效习惯）：高价值 skill 组合固化成 pipeline 不每次重组合。①edge 研究流（edge-pipeline-orchestrator→residual-edge-analyzer→edge-strategy-reviewer→signal-postmortem）②spec→实现流即 §0 SDD 引用不重述。KG：`investing/quantitative-system/agent-skills-catalog.md`（70 skill 索引）+ `agent-working-practices.md`（5 实践+复用指南，已声明融合 §0/§7 不重述）。
