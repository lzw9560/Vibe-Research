...
本项目是**个人投研看板**（自托管、个人使用）。基于公开数据与历史统计特征，系统**可以给出教育研究性判断**——包括模式识别、战法匹配、风险标注、历史规律陈述、研究参考性的推荐与买卖时机研判。但必须守以下边界：

- **教育研究性判断，非投资建议**：系统可呈现"研究参考性"的推荐/买卖时机研判/战法匹配，但必须标注「历史统计特征，研究参考，不构成投资建议」；**不得承诺收益、不得做确定性保证、不得代用户决策**。研究性判断 ≠ 投资建议。
- **判断须可复现**：任何研究性判断须基于公开数据 + 既定规则可复算（可用 `~/tools/financial_rigor.py` 核对），禁止臆造、禁止心算。
- **涨停四池原始池含个股 code/name**，仅供 `market._emotion` 聚合成**不含个股名**的指标（连板梯队/封板率/炸板率/晋级率）；原始池可作为客观榜单呈现，但聚合情绪指标不得泄露个股名。
- **`chat.SYSTEM_PROMPT` 守中立框架**：AI 可给教育研究性分析与研判，但不得做确定性投资建议/收益承诺；新增 AI 工具/改提示词时不得引入收益承诺或代客决策指令。
- **用户私有数据**（持仓/研报/API key）只存**项目目录内** `.vibe-research/`（`VR_DATA_DIR` 默认项目根 `.vibe-research/`，见 `backend/vr_paths.py`），**绝不进 git（已 .gitignore）、不上传、不落 home 目录**。
- 规范的"合规自查"栏必须逐条确认未触边界，否则 spec 不予通过。

> 边界变更记录（2026-07-29）：原 §1 要求"只返回客观数据、不推荐/不给买卖时机"。经研判，个人自托管场景下允许系统输出教育研究性判断（战法匹配/买卖时机研判/风险标注），但仍守"不承诺收益、可复现、私有数据隔离、四池聚合不泄露个股"等底层红线。受影响：`chat.SYSTEM_PROMPT` 措辞、UI 可呈现"研究参考性判断"卡片（须带免责声明）、相关 spec 的合规自查栏须按新边界重审。
