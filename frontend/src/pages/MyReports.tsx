...
export function MyReports() {
  // T9：原 useState(useState) + useEffect(load) → useMyReports()。
  // 列表读取走 hook（error 为 Error | undefined），上传/删除/下载仍为直接调用，
  // 成功后用 refetch() 刷新列表；mutation 错误仍存本地 err。
  // 注：useMyReports 经 Opts 参数化 data 已推断为 MyReport[] | undefined，无需 cast。
  ...
  return (
    <div>
      ...
      {errMsg && (
        <GlassCard>
          <div className="p-3 text-sm text-destructive">{errMsg}</div>
        </GlassCard>
      )}
