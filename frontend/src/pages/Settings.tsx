...
export function Settings() {
  ...
  // T9：原 useEffect 内直接 get*().then(set*) → TanStack Query 只读 hook + 同步到本地可编辑 state。
  // 注：hook 返回类型被 query 选项放宽为 {}，就地窄→宽 cast（与 Health.tsx 同模式）。
  const { data: limitUpData, error: limitUpError, refetch: refetchLimitUp } = useLimitUpScreenerParams();
  const { data: auctionData, error: auctionError, refetch: refetchAuction } = useAuctionParams();
  const { data: reviewData, error: reviewError, refetch: refetchReview } = useReviewParams();

  useEffect(() => {
    if (limitUpData) setLimitUpParams(limitUpData);
  }, [limitUpData]);
  useEffect(() => {
    if (auctionData) setAuctionParams(auctionData);
  }, [auctionData]);
  useEffect(() => {
