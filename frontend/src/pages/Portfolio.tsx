...
export function Portfolio() {
  ...
  // 读：usePortfolio（30 分钟自动轮询）。手动刷新按钮保留 api.refreshPortfolio()（POST
  // /portfolio/refresh 会触发后端拉取最新行情，非纯 re-GET），用其返回值 setQueryData 更新缓存。
  // mutation：addHolding/closePosition 成功后 refetch() 重新 GET（行为等价、更简单）；
  // removeHolding/removeClosed 仍 try/catch{ /* ignore */ } 静默吞错。
  ...
  const manualRefresh = async () => {
    setRefreshing(true);
    try {
      // 保留原 load(true) 语义：POST /portfolio/refresh 触发后端刷新行情，
      // 用响应直接更新缓存（setQueryData），避免一次多余 re-GET。
  ...
  const remove = async (c: string) => {
    try { await api.removeHolding(c); await refetch(); } catch { /* ignore */ }
  };
  ...
  const removeClosed = async (i: number) => {
    try { await api.removeClosed(i); await refetch(); } catch { /* ignore */ }
  };
  ...
  return (
    <div>
      ...
      <GlassCard glow>
        ...
        {holdings.length === 0 ? (
          <p className="py-8 text-center text-sm text-muted-foreground/60">{isLoading ? "加载中…" : "还没有持仓记录，用上面的表单添加一笔。"}</p>
        ) : (
