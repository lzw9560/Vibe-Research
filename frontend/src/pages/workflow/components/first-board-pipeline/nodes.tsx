// S179 R3.4 Step 2: FirstBoardPipeline 拆分——8 Node 组件 + WATCHBOOK_SLOTS + currentWatchbookSlot
// 从 FirstBoardPipeline.tsx 抽出，保持 verbatim 不改逻辑。
import { useState } from "react";
import { cn } from "@/lib/utils";
import type { FirstBoardCandidatesResponse, FirstBoardExcludedItem } from "@/lib/api";
import { NODE, ArrowDown, FunnelShrinkBar } from "@/components/pipeline/primitives";
import { NODE_DASHED, NODE_GREEN, NODE_AMBER, NODE_RED } from "./styles";
import { CandidateScoreTable } from "./CandidateScoreTable";


// ---- ① 筛选节点：涨停股池 → 首板过滤 → 3层剔除 ----
export function FilterPipelineNode({ data }: { data: FirstBoardCandidatesResponse | null }) {
...
export function ConfirmNode({ data }: { data: FirstBoardCandidatesResponse | null }) {
  const candidates = data?.candidates ?? [];
...
export function PositionNode({ data }: { data: FirstBoardCandidatesResponse | null }) {
  const candidates = data?.candidates ?? [];
  return (
...
// ---- ⑤ 飞书通知状态栏（075）----
export function FeishuStatusBar() {
  const statuses = [
