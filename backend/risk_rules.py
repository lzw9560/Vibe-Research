...
1. 风险宪法——使用者自己写下的阈值（单笔/单日最大亏损、最大持仓数、连亏降档…）。
   系统只监控有没有违反他自己写的规则，不给推荐值。
2. 权益曲线状态——距高点回撤多深多久、多少笔没创新高、连亏分布，以及去掉最好的 1 笔 /
   3 笔之后还剩多少。
3. 纪律损益归因——只保留标了「按计划」的交易时，曲线会是什么样。

## 边界

全部只统计使用者自己录入的交易数据，不涉及任何证券的分析、预测或建议。不产出"今天该不该
做"，只显示"你写下的规则是什么、有没有遵守、违反之后结果如何"。
...
S166 fresh-impl（design-agnostic）。equity_curve/discipline/violations/rolling 算法参考自
cb54a96 历史，非整文件复活。
"""
...
# 风险宪法的默认值。全部是"用户自己的规矩"，不是推荐数值——不同资金量、不同打法的合理
# 阈值差得远，这里只给一套能跑起来的初值。
DEFAULT_RULES = {
...
def equity_curve(trades: list[dict]) -> dict:
    ...
    累计盈利多少没什么信息量。真正要看的是：距高点回撤多深、回撤多久没恢复、多久没创新高、
    连亏几次、盈利是不是高度依赖少数几笔。
    """
    ...
    return {
        ...
        # ⚠️ 盈亏恰好为 0 的笔不进分母（同 journal 战绩与 rolling 的口径）：持平既不是赢也不是
        #    输，算进分母会把胜率稀释。
...
def discipline(trades: list[dict]) -> dict:
    ...
    return {
        ...
        "note": ("「计划外」的交易需要你自己在录入时标注——没标注的归到未标注，不猜。"
                 "这不是市场分析，是你自己的行为统计。"),
    }
...
def violations(trades: list[dict], rules: Optional[dict] = None) -> dict:
    ...
    ⚠️ 这里只对照用户写下的规则，不替他判断该不该交易。"今天不该做"这种话只能由他自己
    的规则得出，系统负责执行与提醒。
    """
    ...
    # ⚠️ 每条规则都要报「查了没有」。只给一个总违规数的话，规则明明配了却从没被检查过，
    #    界面上会显示成"0 次违反"——使用者以为自己守住了，其实那条根本没跑。
    ...
    for t in trades:
        st = t.get("settled") or {}
        # ⚠️ 优先用按成交日拆分的账。整笔累计额全挂到 last_sell 的话，分两天减仓时两天盈亏
        #    会并到后一天，这条规则就查不准。
...
# ---------------------------------------------------------------- 滚动窗口
# ⚠️ 终身统计会把最近的退化藏起来。前 150 笔赚钱、最近 50 笔一直亏，终身数字照样漂亮。
# 短线最近这批才是有效样本。故除终身外另给近 10/20/50 笔三个窗口。
_WINDOWS = (10, 20, 50)
_MIN_TREND_WINDOW = 10


def _window_stats(closed: list[dict]) -> dict:
    """一个窗口内的核心读数。closed 必须已按平仓日排好序。"""
    pnls = [float(t["settled"]["realized_pnl"]) for t in closed]
    if not pnls:
        return {"trades": 0}
    wins = [v for v in pnls if v > 0]
...
def rolling(trades: list[dict]) -> dict:
    """终身 + 近 10/20/50 笔的并排对比。窗口按平仓日取最后 N 笔。"""
    closed = [t for t in trades
              if (t.get("settled") or {}).get("realized_pnl") is not None]
    if not closed:
        return {"available": False, "reason": "还没有已平仓且填了成交明细的交易"}
    ...
    out["note"] = ("10 笔看状态、20 笔看节奏、50 笔看打法是否还成立。"
                   "终身统计会把最近的退化藏起来——前面赚够了，最近一直亏，终身数字照样漂亮。")
    return out
...
def render(rep: dict) -> str:
    ...
    ⛔ 不要接进任何 AI prompt。个人持仓与盈亏一旦进 prompt，模型的回答就变成"针对这个人当前
    处境"的意见，那正是个性化投资建议，是本项目合规立足点（非个性化）唯一不能碰的那条线。
