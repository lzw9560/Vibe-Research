...
⚠️ 这里没有任何外部标准："仓位偏大"是相对他自己的中位仓位，"持有偏久"是相对他自己的
中位持有天数；规则违反用 risk_rules.load_rules() 里他自己写下的阈值。把行业经验塞进阈值
就变成了在教他怎么交易。

## ⚠️ 不排序"严重程度"，也不给建议

每条只说哪里不寻常，不说"所以应该怎样"。排序按日期倒序，不按任何主观的严重程度。
...
S166 fresh-impl（design-agnostic）。build 算法参考自 cb54a96 历史，非整文件复活。
"""
...
# 偏离多少倍中位数才算"不寻常"。⚠️ 不是"多少算大"的行业标准，只是"相对他自己明显偏离"
# 的门槛——2 倍是个宽松的筛子，宁可少报不多报。
_DEVIATION_X = 2.0
...
def _capital_of(t: dict) -> Optional[float]:
    st = t.get("settled") or {}
    # ⚠️ 字段名是 amount（journal._settle 里的"建仓总金额 = 这笔占用的资金"），不是 capital_used。
    # 写错的话主路径恒失效、悄悄退到下面按 fills 重算——结果还对，但那段查表白写了，而且下次
    # 有人改 fills 结构就会静默出错。
...
def build(limit: int = 500) -> dict:
    ...
    # try 在循环内——单笔缓存损坏抛异常不中断全循环（否则 exc_skipped 系统性偏低、
    # excursion_hint 提示不准）。
