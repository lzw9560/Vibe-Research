# -*- coding: utf-8 -*-
"""个人交易日志：记录每一笔交易，并附上当时的市场环境快照。

每笔记录自动带上成交那天的情绪档位、赚钱效应、以及该股当天的客观状态
（几板、几点封板、炸板次数、所属题材），用于事后按环境/打法/是否按计划分组自查。

## 边界

只统计使用者自己录入的历史行为，不产出选股、买卖时机或参与倾向。
...
S166 fresh-impl（design-agnostic）。_settle 移动加权平均算法参考自 cb54a96 历史，
非整文件复活——文件结构、注释、接线按当前 develop 重写。
"""
...
from utils.journal_util import atomic_write_json, china_now, validate_trade_date
...
# 交易记录结构版本。
# v2 起一笔交易 = 多次成交（fills），支持分批建仓、做 T、买卖不同日。
...
# 读-改-写必须串行。原子写只保证单次写入不会半截，挡不住两个请求各读一份、各追加一条、
# 后写的覆盖先写的——那是静默丢单。
...
def _load_raw() -> list[dict]:
    ...
    got = d.get("schema")
    if got != _SCHEMA:
        # 能自动迁移的就迁，别让老用户卡死。迁移前先备份——账本是永久数据，宁可多留一份。
...
def _migrate(from_schema, trades: list[dict]) -> Optional[list[dict]]:
    ...
    v1 → v2：v1 没有成交明细。不伪造 fills——价与量当时没记录，补空结构即可。
    v2 → v3：新增计划退出边界。老记录一律补 None，不反推——计划止损位是下单时的主观意图，
...
def _market_context(date: str) -> dict:
    """那天的市场环境。从 daily_review 磁盘持久化层读（盘后 precompute 落盘时零网络盖章）。

    Vibe-Research 的 ReviewReport 字段：sti_phase（≈emotion_phase）/money_effect_median
    （precompute_daily 算）/zt_total（≈limit_up_count）。promotion_overall/
    never_broken_rate 无对应字段——诚实 None，不臆造。
    ...
    return {
        ...
        "promotion_overall": None,          # ReviewReport 无此字段，不臆造
        "limit_up_count": review.get("zt_total"),
        "never_broken_rate": None,           # 无此字段，不臆造
...
def _norm_fills(fills: list[dict]) -> list[dict]:
    ...
    for i, f in enumerate(fills or []):
        ...
        # 可选：这笔成交的实际费用（对账单上的数）。填了就以它为准，不再按费率估。
    ...
    #    成交明细没有时间字段，同日的先后只能靠录入顺序表达——若把同日的买入一律排到卖出
    #    之前，"买→卖→买→卖"这种同日做 T 会被压成"买买卖卖"，两轮变成一轮，
    #    周期数与每轮的成本基准都不对了。代价：同日先录卖出再录买入会被判成超卖——
    #    那本来就是录入错误，报错是对的。
...
def _settle(fills: list[dict], fees_cfg: Optional[dict] = None) -> dict:
    ...
    - **移动加权平均成本**（与券商对账口径一致）。每次卖出按卖出那一刻的持仓均价结转成本，
      剩余持仓的均价不因这次卖出而改变。
    - 持仓归零后再买入 = 开启新的持仓周期；已实现盈亏累加，成本重置。
    - avg_cost：当前持仓的加权成本（在险资金 / MFE-MAE / 收件箱都按这个口径读）。
      已全部平仓时给已实现部分的加权成本，让这些消费方仍有成本基准可用。
    - amount：整个过程中的峰值占用资金（这笔最多占用了多少钱），不是买入总额。
    - 只算已实现部分，未卖出的不计浮盈浮亏。
    - realized_pnl 是净额（已扣费用）。毛额另给 gross_pnl，费用给 fees。
      ⚠️ 对高换手的短线，佣金+印花税+过户费不是小数：一堆薄利交易在计费后可能接近持平
      甚至转亏，胜率与期望会被系统性高估。买入费用按卖出比例结转。
    ...
    realized_pnl = 0.0      # 累计已实现毛盈亏
    realized_cost = 0.0     # 已实现部分对应的成本（算百分比的分母）
    realized_shares = 0.0
    realized_fee = 0.0      # 已实现部分对应的费用（买入分摊 + 卖出）
    # ⚠️ 按每笔卖出发生的那一天记净盈亏。整笔累计额全挂到最后一次卖出的话，分两天减仓时
    #    两天的盈亏会被并到后一天——「单日最大亏损」这条规则就查不准。
    ...
    if sells:
        ...
        # realized_pnl 是净额——下游全部统计（权益曲线/胜率/盈亏比/模式卡/归因）都读它，
        # 让它带费用才是使用者真正赚到的钱。
...
def add_trade(date: str, code: str, name: str, playbook: str,
    ...
    - 填成交明细 fills：[{side:buy/sell, date, price, shares}, …]——支持分批建仓、
      做 T、隔日卖出。系统自动算加权成本、已实现盈亏、盈亏金额、持有天数、占用资金。
      ⚠️ 只算已实现部分，不虚构浮盈。

    as_planned = 这笔是否按计划执行。
    planned_stop / planned_target = 下单时写下的计划止损价 / 目标价。⚠️ 必须是当时写下的值；
    在险资金按 planned_stop 计算，事后补填会让该口径失真。没写就留空。
    """
...
def update_trade(trade_id: str, *, fills=None, note=None, as_planned=_UNSET,
    ...
    """更新一笔已有交易——主要用途是给持仓中的记录补上后来的成交。

    只能改传进来的字段，其余原样保留。传了 fills 就按新的明细整份重算。

    ## ⚠️ 计划边界改了要留痕

    在险资金的全部意义建立在"止损是下单时写下的"。事后改动不禁止（填错了要能纠正），
    但必须记下 planned_edited_at，让读数的人知道这个值不再是当时那个。
    """
    with _LOCK:
        trades = _load_raw()
        ...
        if fills is not None:
            ...
            # ⚠️ 算不出来时要清掉旧值，不能留着：撤掉卖出之后如果 pnl_pct 还挂着，
...
def all_trades() -> list[dict]:
    ...
    ⚠️ 持仓聚合与风控统计必须走这个：用 list_trades(limit=N) 的话，账本超过 N 条之后，
    较早但仍未平仓的记录会被静默漏掉——持仓少一只、在险资金偏小，而两边都不会报错。
    """
...
def _bucket_stats(rows: list[dict]) -> dict:
    ...
    有成交明细的另外汇总盈亏金额——百分比与金额是两个口径，仓位不同时二者结论可能相反。
    """
    ...
    return {
        **base,
        # ⚠️ 持平（0%）不计入胜率分母——分母只含分出胜负的笔数。全部持平时分母为 0，
        #    胜率给 None（不是 0）——那是"没有胜负可言"，不是"没赢过"。
