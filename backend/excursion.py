...
# P3-T3d：util→vibe_astock_util；数据路径→vr_paths.resolve_data_dir
from utils.vibe_astock_util import atomic_write_json
from vr_paths import resolve_data_dir

# P3-T3c 改写：bars() 原裸调 akshare stock_zh_a_hist_tx（qt.gtimg.cn，无防封）→
# astock.kline_multi（多源回退 baidu→sina→mootdx→akshare，em_get/限流防封）。
import astock
...
def for_trade_cached_only(trade: dict) -> Optional[dict]:
    ...
    接口会卡几分钟甚至超时。inbox 的 MFE/MAE 判定走本函数（零网络）。
    """
...
def bars(code: str, start: str, end: str) -> Optional[list[dict]]:
    ...
    P3-T3c 改写：原 akshare stock_zh_a_hist_tx(symbol, start, end)（裸调 qt.gtimg.cn
    无防封）→ astock.kline_multi(code)（多源回退防封）。kline_multi 只取 code 无日期范围
    （返默认窗口），故 filter 到 [start, end]；窗口未覆盖老交易的日期段时返 None
    （诚实 unavailable，不臆造——防封优先于历史全覆盖）。
    """
    p = _cache_path(str(code).zfill(6), start, end)
    if os.path.isfile(p):
        try:
            ...
            pass
    try:
        # S149 P3 审查修复：强制 adjust="none"（raw 口径）—— avg_cost 来自用户原始成交价
        # （raw），若用默认链（baidu/akshare qfq + sina/mootdx raw 混用）会口径不匹配：
        # qfq high 与 raw cost 算 MFE 系统性低估/高送转甚至变号；且口径随哪个源先可达而变
        # （不可复现，违背 §1.2）。adjust="none" 只走 raw 源（sina/mootdx），口径对齐成本。
