...
def _compute_rotation_speed(current_ranking: list[dict], prev_ranking: list[dict]) -> float:
    """计算板块轮动速度 (0-100)。"""
...
async def calculate_sector_divergence(date: str | None = None) -> list[SectorDivergence]:
    ...
    try:
        ...
        sectors = sector_data.get("top", []) + sector_data.get("bottom", [])
        if not sectors:
            return []
        ...
        for sector in sectors:
            ...
            change_pct = sector.get("change_pct", 0) or 0
            up_count = sector.get("up_count", 0) or 0
            down_count = sector.get("down_count", 0) or 0
...
async def calculate_sector_rotation(date: str | None = None) -> SectorRotation | None:
    ...
    try:
        ...
        today_sectors = today_data.get("top", []) + today_data.get("bottom", [])
        ...
        # 识别热点/冷门板块
        hot = [s["name"] for s in today_sectors[:5] if s.get("change_pct", 0) > 0]
        cold = [s["name"] for s in today_sectors[-5:] if s.get("change_pct", 0) < 0]

        return SectorRotation(
            date=display_date,
            sectors=today_sectors[:20],  # 只保留前 20
