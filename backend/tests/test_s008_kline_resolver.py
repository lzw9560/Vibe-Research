...
def test_fallback_on_exception(monkeypatch):
    ...
    monkeypatch.setattr(kr, "_baostock", _stub("baostock"))
    bars, src = kr.fetch_kline("600519")
    assert src == "sina"
    assert len(bars) == 1
...
def test_all_fail_returns_empty_honest(monkeypatch):
    ...
    monkeypatch.setattr(kr, "_baostock", _stub("baostock", bars=[]))
    bars, src = kr.fetch_kline("600519")
    assert bars == []
    assert src is None
...
def test_list_sources_order():
    """源链顺序：独立源在前，baostock 兜底（本环境唯一可靠源，无 IP 限制免防封）。"""
    assert kr.list_sources() == ["baidu", "sina", "mootdx", "baostock"]


def test_list_sources_by_adjust():
    """list_sources(adjust) 按口径筛选——qfq 只返百度+baostock，none 返新浪+mootdx。"""
    assert kr.list_sources(adjust="qfq") == ["baidu", "baostock"]
...
def test_adjust_qfq_falls_to_baostock_not_raw(monkeypatch):
    """百度抛异常 → 跳过新浪/mootdx（raw，口径不符）→ 命中 baostock（qfq）。
    关键：不回退 raw 源——混用口径会污染收益。"""
    monkeypatch.setattr(kr, "_baidu", _stub("baidu", exc=ConnectionError("banned")))
    monkeypatch.setattr(kr, "_sina", _stub("sina", bars=[{"date": "d1", "close": 999.0}]))
    monkeypatch.setattr(kr, "_mootdx", _stub("mootdx", bars=[{"date": "d1", "close": 999.0}]))
    monkeypatch.setattr(kr, "_baostock", _stub("baostock", bars=[{"date": "d1", "close": 10.0}]))
    bars, src = kr.fetch_kline("600519", adjust="qfq")
    assert src == "baostock"   # sina/mootdx 被口径过滤掉，不是被试过再跳过


def test_adjust_qfq_no_source_returns_empty_honest(monkeypatch):
    """百度+baostock 都失败 → adjust='qfq' 诚实返空，**不**回退到 raw 源（新浪/mootdx 可用也不取）。
    不臆造复权因子重算 raw→qfq。"""
    monkeypatch.setattr(kr, "_baidu", _stub("baidu", exc=ConnectionError("x")))
    monkeypatch.setattr(kr, "_baostock", _stub("baostock", exc=ConnectionError("x")))
...
def test_adjust_with_sources_subset(monkeypatch):
    """sources + adjust 复合筛选：sources 限 baostock，adjust=qfq → 仍命中 baostock。
    sources 排除 baostock → adjust=qfq 无可用源返空。"""
    monkeypatch.setattr(kr, "_baostock", _stub("baostock", bars=[{"date": "d1", "close": 10.0}]))
    monkeypatch.setattr(kr, "_baidu", _stub("baidu"))
    bars, src = kr.fetch_kline("600519", sources=["baostock"], adjust="qfq")
    assert src == "baostock"
