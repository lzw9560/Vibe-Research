# -*- coding: utf-8 -*-
"""TDD tests for regime_stratified_leader_drop_reversal_lift harness (AAA pattern).
...
edge_type=event -- S204 T5b (fb2cc65) event_drift fix applied.
"""
...
from tools import regime_stratified_leader_drop_reversal_lift as harness
...
def test_run_produces_verdict_per_regime(monkeypatch):
    ...
    assert "leader_drop_reversal_regime:bull" in line_ids
    assert "leader_drop_reversal_regime:bear" in line_ids
    assert "leader_drop_reversal_regime:range" in line_ids
...
def test_run_skips_regime_with_too_few_picks(monkeypatch):
    ...
    assert len(calls) == 1
    assert calls[0]["line_id"] == "leader_drop_reversal_regime:bull"
