...
class ReviewReport(BaseModel):
    ...
    # money_effect 中位数——precompute_daily 算并落盘（cycle_position 不进盖章，双源规则）。
...
class DailyReviewer:
    ...
    def _get_sti_data(self, trade_date: str) -> dict[str, Any]:
        """获取 STI 情绪数据。"""
        try:
            # S149 P3 审查修复：backend 非 package（无 __init__.py，uvicorn 从 backend/
            # 以 app:app 启动）→ `from backend.limitup_sti` 生产 ModuleNotFoundError 被裸
            # except 吞掉 → sti_phase 恒 None → journal.market.emotion_phase 盖章坏。
            # 改 `from limitup_sti import`（同进程根包路径，已验证可用）。
    ...
    def precompute_daily(self, date: str) -> ReviewReport:
        ...
        供 journal._market_context 零网络盖章读取（generate_review 同步打东财，违背 journal 零网络契约）。
        """
        result = self.generate_review(date)

        # P3-T7a 盖章字段：money_effect 中位数（emotion_metrics_ext，em_zt_topic_pool 防封）。
        # 取不到不阻塞复盘（median=None，journal 如实记 None）。
        try:
    ...
    def get_daily_review(self, date: str) -> dict | None:
        """S149 P3 critical #1：先读内存缓存 → 磁盘 → fallback precompute_daily（含网络）。

        journal._market_context 走这里——盘后复盘已 precompute 落盘时**零网络**盖章。
        三级读：_CACHE（内存快路径，12h TTL）→ 磁盘 JSON → fallback 重算（触网）。
        磁盘未命中（precompute 没跑/损坏）才 fallback（此时触网，非 journal 理想路径，
        但不臆造——如实 fallback + 落盘供下次零网络读）。
        """
        cache_key = f"daily_review_{date}"
        # S149 P3 审查修复：_CACHE 内存快路径（消除 write-only 死代码——precompute_daily
        # 写 _CACHE，此处读，命中免磁盘 IO）
...
def _daily_review_path(date: str) -> str:
    """S149 P3：复盘报告磁盘持久化路径 <VR_DATA_DIR>/daily-review/<date>.json。
...
def get_daily_review(date: str) -> dict | None:
    ...
    （守 journal.py:14 零网络契约 + em_get 防封底线）。
    """
