...
def calc_total_score(factors: dict[str, float], weights: str = "full") -> float:
    ...
    weights:
        "full"（默认）: S047 阶段 B 改稿（149 日 5267 样本回测证据，specs/S047-基因分权重回测校准/证据报告.md）：
            次日溢价率(40%) + 红盘率(25%) + 封板率(25%) + 炸板后溢价(0%) + 涨停频次(10%)。
            原 25/25/25/15/10——炸板后溢价 live 数据零方差（15% 权重纯浪费、压顶 total_score ~52），
            次日溢价率为最强单调因子（hit 60.8%→70.9%→82.3%）升权。
        "rebuild": 三维重建（K 线不可推封板率/炸板后溢价）——次日溢价率(40%) + 红盘率(40%) + 涨停频次(20%)
            （S047 grill 锁定：rebuild 权重无替代证据，维持不动）
    """
    w = {
        "full": {"次日溢价率": 0.40, "红盘率": 0.25, "封板率": 0.25, "炸板后溢价": 0.00, "涨停频次": 0.10},
        "rebuild": {"次日溢价率": 0.40, "红盘率": 0.40, "涨停频次": 0.20},
    }.get(weights, {"次日溢价率": 0.40, "红盘率": 0.25, "封板率": 0.25, "炸板后溢价": 0.00, "涨停频次": 0.10})
