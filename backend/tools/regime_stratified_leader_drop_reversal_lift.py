#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Leader-drop reversal (龙头大跌反包) x MA20 3-way regime stratified S44 harness.

Question: is leader-drop reversal overnight return a regime artifact?
...
edge_type=event -- S204 T5b (fb2cc65) event_drift fix applied; bull-market
base_rate=0 drift 假阳性 corrected (event_mean vs universe_mean two-sample).
...
SCRIPT_PATH: str = "tools/regime_stratified_leader_drop_reversal_lift.py"

STRATEGY_NAME: str = "leader_drop_reversal"
...
def run(
    *,
    ...
    """Run per-regime S44v2 event verdict for leader-drop reversal.
...
def main() -> int:
    """Real-data entry point (requires leader-drop reversal picks + regime cache).
