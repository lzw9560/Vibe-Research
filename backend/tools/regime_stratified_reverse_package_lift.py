#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Reverse-package (zhaban fanbao) x MA20 3-way regime stratified S44 harness.

Question: is reverse-package (failed-seal reversal) overnight return a
regime artifact?  Splits picks by MA20 3-way regime (bull/bear/range),
then computes per-regime S44v2 event verdict via wire_verdict.
...
SCRIPT_PATH: str = "tools/regime_stratified_reverse_package_lift.py"

STRATEGY_NAME: str = "reverse_package"
...
def run(
    *,
    ...
    """Run per-regime S44v2 event verdict for reverse-package (zhaban fanbao).
...
def main() -> int:
    """Real-data entry point (requires reverse-package picks + regime cache).
