...
def run_platform_breakout_lift():
    ...
    # ── S168 接线：6 verdict（3 arm × 2 regime）落 Recorder + lineage ──
    _FROZEN_S168 = "74295b9"  # pre-registered 冻结 commit
    _s168_arms = [
        ("tight", tight_by_day, raw_by_day),
        ("confirm", confirm_by_day, raw_by_day),
        ("both", both_by_day, raw_by_day),
        ("tight_bull", tight_bull, raw_bull),
        ("confirm_bull", confirm_bull, raw_bull),
        ("both_bull", both_bull, raw_bull),
    ]
    s168_verdicts = {}
    for _arm_name, _surv, _raw in _s168_arms:
        ...
        for _d, _rs in _surv.items():
            for _r in _rs:
                ...
                _dts.append(_d)
        if len(_rets) < 2:
            print(f"[S168] platform_breakout:{_arm_name} skips (n<2)")
            continue
        _v = wire_verdict(
            line_id=f"platform_breakout:{_arm_name}",
            returns=_rets,
            dates=_dts,
            edge_type="selection",
