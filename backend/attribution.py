...
系统里两套验证各回答半个问题：reflection / verification 说昨晚那份判断今天成立没有，
journal / risk_rules 说这些天赚了还是亏了。只看一层会得出错的结论——看对了却亏钱和看错了
才亏钱要修的地方不同（前者改执行，后者改判断）。
...
交易在 D 日入场，依据是 D-1 晚那份判断；reflection 文件里 prediction_date = D-1、
eval_date = D。所以对齐关系是：交易的入场日（settled.first_buy）←→ reflection 的 eval_date。

⚠️ 不能按平仓日对齐：D 日买、D+2 日卖的交易，按平仓日会去比 D+1 晚做的判断——那份判断没参与
这笔决策，归因张冠李戴，而两个数都长得正常、看不出来。
...
Vibe-Research 暂无 reflection/预测命中数据源（reflection.py 是流式 LLM 调用，语义不同；
prediction_ledger 格式未对齐）。故 _read_hits 诚实返空 → attribution() 降级 available:False
（"还没有可用的市场判断记录"），不臆造判断命中。函数结构已就位，待新 spec 接预测命中数据源后
激活。

S166 fresh-impl（design-agnostic）。四格算法参考自 cb54a96 历史，非整文件复活。
"""
...
def _read_hits() -> dict[str, dict]:
    ...
    ⚠️ Vibe-Research 暂无此数据源（reflection.py 语义不同、prediction_ledger 格式未对齐）
    → 诚实返空，不臆造判断命中。待新 spec 接预测命中数据源后激活。
    """
    return {}
...
def attribution(limit: int = 500) -> dict:
    ...
    ⚠️ 只用已平仓且填了成交明细的交易——没有金额就没有"那天赚没赚"，拿百分比按笔数平均
    会把 1 万的仓和 1 千的仓当成一样重。
    """
    import journal
...
def render(rep: dict) -> str:
    ...
    ⛔ 不要把它接进任何 AI prompt。个人持仓与盈亏一旦进 prompt，模型的回答就变成
    "针对这个人当前处境"的意见——那正是个性化投资建议，是本项目合规立足点（非个性化）
    唯一不能碰的那条线。
    """
