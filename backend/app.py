...
from routers import earnings_calendar as ec_router  # S216 P2：财报季日历聚合
...
async def lifespan(_app: FastAPI):
    ...
    try:
        ...
        _fw_started = _threading_mod.Event()
        def _start_fw():
            try:
                ...
                _fw_started.set()
        _fw_thread = _threading_mod.Thread(target=_start_fw, name="feishu-ws-init", daemon=True)
        _fw_thread.start()
...
app.include_router(ec_router.router)  # S216 P2：财报季日历聚合
app.include_router(wechat_bot_router.router)  # 微信 Bot 双向对话（企业微信回调 + KG 工具）
app.include_router(backtest.router)
app.include_router(bidding.router)
app.include_router(fusion_router.router)  # S194 FE-6：融合研判查询
